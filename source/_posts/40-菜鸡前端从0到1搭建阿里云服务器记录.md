...
5. 能够跟公司解耦。张一鸣说过：“年轻人要有随时离开公司的能力，也要有永不离开的态度。”
   可以弄懂要上线一个产品的整个技术链路，比如申请域名，https 证书，相关权限审核，部署网站，部署服务。以作自身的技术储备之用。
...
```
docker run -p 3306:3306 --name mysql -v /var/mysql/data/:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=`你的数据库密码` -d --privileged=true --restart=always hub.c.163.com/library/mysql:latest
```
