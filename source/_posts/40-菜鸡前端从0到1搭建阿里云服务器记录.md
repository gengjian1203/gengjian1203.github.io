...
先在本地创建一个 `/usr/local/mongo/mongod.cfg` 文件, 用来在宿主环境报错对应的配置

```bash
docker run --name mongo -p 222222:27017 -v /usr/local/mongo/data:/data/db -v /usr/local/mongo/mongod.cfg:/etc/mongod.cfg -e TZ=Asia/Shanghai -d --restart=always --privileged=true mongo:4.2
```
